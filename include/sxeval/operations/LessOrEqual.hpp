...
public:
    ...
    inline LessOrEqual(const std::vector<IInstruction<T>*>& args) :
        AOperation<T>(args) {}
...
template <typename T>
constexpr const char* sxeval::operations::LessOrEqual<T>::KEY;
...
void sxeval::operations::LessOrEqual<T>::execute() {
    ...
    while (verif && (i + 1) < this->_args.size()) {
        verif = sxeval::LessOrEqual(this->_args[i].get(),
            this->_args[i + 1].get());
        ++i;
    }
    if (verif) {
