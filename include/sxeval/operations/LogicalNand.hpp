...
public:
    ...
    inline LogicalNand(const std::vector<IInstruction<T>*>& args) :
        AOperation<T>(args) {}
...
template <typename T>
constexpr const char* sxeval::operations::LogicalNand<T>::KEY;
...
void sxeval::operations::LogicalNand<T>::execute() {
    ...
    while (verif && (i + 1) < this->_args.size()) {
        verif = sxeval::LogicalNand(this->_args[i].get(),
            this->_args[i + 1].get());
        ++i;
    }
    if (verif) {
