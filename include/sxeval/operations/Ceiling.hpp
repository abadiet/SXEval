...
public:
    ...
    inline Ceiling(const std::vector<IInstruction<T>*>& args) :
        AOperation<T>(args) {}
...
template <typename T>
constexpr const char* sxeval::operations::Ceiling<T>::KEY;

template <typename T>
void sxeval::operations::Ceiling<T>::execute() {
    this->_result = static_cast<T>(std::ceil(this->_args.front().get()));
}
