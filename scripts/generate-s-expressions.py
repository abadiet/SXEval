...
def generate_expression(operations, max_depth, ratio_atom, ratio_value, ratio_encapsulated, current_depth=0):
    ...
    max_arity = op["max_arity"] if op["max_arity"] != -1 else (min_arity + random.randint(0, 4))
    arity = random.randint(min_arity, max_arity)

    args = [generate_expression(operations, max_depth, ratio_atom, ratio_value, ratio_encapsulated, current_depth + 1) for _ in range(arity)]
